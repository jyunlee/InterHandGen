...
class ManoLayer(Module):
    ...
    def axis2pca(self, axis):
        # axis: bs x 45
        pca = axis - self.hands_mean.to(axis.device)
        pca = pca.mm(self.hands_components_inv.to(axis.device))
        return pca
    ...
    def forward(self, root_rotation, pose, shape, trans=None, scale=None):
        ...
        self.hands_components = self.hands_components.to(pose.device)
        self.hands_mean = self.hands_mean.to(pose.device)
        self.shapedirs = self.shapedirs.to(pose.device)
        self.v_template = self.v_template.to(pose.device)
        self.J_regressor = self.J_regressor.to(pose.device)
        self.posedirs = self.posedirs.to(pose.device)
        self.weights = self.weights.to(pose.device)
